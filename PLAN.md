...
## M10 — энкодер ViT-L/14 (1024d)

Цель: продолжить линию «энкодер важнее тюнинга» RN50 → ViT-S → ViT-B.

- [x] Поддержка 1024d (`zf/encoders/dinov2.py`: `dinov2_vitl14`), скрипт
      `benchmarks/extract_vitl.py` с чекпоинтингом по чанкам (долгий прогон
      ~2.5–3 ч CPU, убийство процесса теряет максимум один чанк)
- [x] argv-кэши в `run_m9_dinov2b.py` (переиспользование без нового скрипта)
- [ ] Извлечение base + aug4 (`data/vitl_1024.npz`, `data/vitl_1024_aug4.npz`),
      верификация aug4↔base cosine=1.000000
- [ ] Чистая память k8/k16 на base (M5-стиль)
- [ ] Гибрид α=0.2 k16 на aug4 (M6c-стиль)
- [ ] Открытый мир M7 merge@0.45 на base
- [ ] Чемпион → `champions/` (при победе над M9)
- [ ] Вердикт в `BENCHMARK_LOG.md`, коммит

**Definition of done:** честный прогон на верифицированном кэше;
линия энкодеров продолжена цифрой, вердикт зафиксирован.
