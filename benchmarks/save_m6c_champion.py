#!/usr/bin/env python3
"""Сохранение чемпиона M6c: гибрид память+probe (α=0.2) на DINOv2 aug4."""
...
CACHE = ROOT / "data" / "dinov2_384_aug4.npz"
...
def main() -> None:
    ...
    for phase_idx, classes in enumerate(PHASES):
        ...
        score = ALPHA * norm(np.stack([P_probe[:, col[c]] for c in seen], 1)) \
            + (1 - ALPHA) * norm(S_mem)
        ...
        print(f"phase {phase_idx}: acc={phase_acc[-1]*100:.2f}% "
              f"protos={mem.size}")

    forg = [max(v[:-1]) - v[-1] for v in hist.values() if len(v) >= 2]
    result = {
        "config": {"cache": "dinov2_384_aug4", "topk": TOPK, "alpha": ALPHA},
        "p_last": phase_acc[-1],
    ...
    (ROOT / "results" / "m6c_champion.json").write_text(
        json.dumps(result, indent=2))

    out = ROOT / "champions" / "m6c_hybrid_dinov2aug4_topk16_a02.npz"
    np.savez_compressed(out,
                        prototypes=mem.prototypes[:mem.size],
                        labels=mem.labels[:mem.size],
                        counts=mem.counts[:mem.size])
    print(f"\nЧемпион сохранён: {out}")
